...
export function getLimitsColumns(
	limitsMutate: () => void,
): ColumnDef<NatsAuthLimitsRecord>[] {
	return [
		{
			id: "name",
			accessorKey: "name",
			...
			cell: ({ row }) => {
				return (
					<div>
						...
						row.original.jetstream_max_disk === undefined
							? "Unlimited"
			...
			cell: ({ row }) => {
				return (
					<div>
						...
						row.original.jetstream_max_memory === undefined
							? "Unlimited"
							: toStringSigBytesPerKB(
									row.original.jetstream_max_memory,
									2,
									1024,
								)}
					</div>
